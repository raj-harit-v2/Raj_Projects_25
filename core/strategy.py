...
def select_decision_prompt_path(planning_mode: str, exploration_mode: Optional[str] = None) -> str:
    ...
    if planning_mode == "conservative":
        return "prompts/New_Decision_Prompt.txt"  # Using new optimized prompt
    ...
    return "prompts/New_Decision_Prompt.txt"  # safe fallback to new prompt
...
async def decide_next_action(
    ...
    historical_context = getattr(context, "historical_context", "No relevant historical context available.")
...
async def conservative_plan(
    ...
    if force_replan or not filtered_summary.strip():
        log("strategy", "[WARNING] Force replan or no filtered tools. Using all tools.")
...
async def exploratory_plan(
    ...
    if force_replan:
        log("strategy", "[WARNING] Force replan triggered. Attempting fallback.")

        if memory_fallback_enabled:
            fallback_tools = find_recent_successful_tools(memory_items)
            if fallback_tools:
                log("strategy", f"[SUCCESS] Memory fallback tools found: {fallback_tools}")
                ...
                log("strategy", "[WARNING] No memory fallback tools. Using all tools.")
    ...
    if not filtered_summary.strip():
        log("strategy", "[WARNING] No filtered tools. Using all tools.")
        tool_context = summarize_tools(all_tools)
    else:
        tool_context = filtered_summary
