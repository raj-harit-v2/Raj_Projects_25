...
class MemoryItem(BaseModel):
    ...
    metadata: Optional[dict] = {}  # Optional metadata field
    ...
    def add_tool_success(self, tool_name: str, success: bool):
        ...
        for item in reversed(self.items):
            if item.tool_name == tool_name and item.type in {"tool_call", "tool_output"}:
                ...
                log("memory", f"[SUCCESS] Marked {tool_name} as success={success}")
                self.save()
                return

        log("memory", f"[WARNING] Tried to mark {tool_name} as success={success} but no matching memory found.")
