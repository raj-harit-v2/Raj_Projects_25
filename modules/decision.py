...
async def generate_plan(
    ...
    step_num: int = 1,
    max_steps: int = 3,
    historical_context: str = "No relevant historical context available."
) -> str:
    ...
    # Check if prompt supports historical_context parameter
    if '{historical_context}' in prompt_template:
    ...
    try:
        ...
        if re.search(r"^\s*(async\s+)?def\s+solve\s*\(", raw, re.MULTILINE):
            ...
            log("plan", "[WARNING] LLM did not return a valid solve(). Defaulting to FINAL_ANSWER")
            return "FINAL_ANSWER: [Could not generate valid solve()]"


    except Exception as e:
        log("plan", f"[ERROR] Planning failed: {e}")
